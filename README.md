...
제가 구현하고자 했던 기능은 정류장을 검색하면 해당 정류장의 버스의 노선번호와 남은 정류장 숫자를 표시하고 다음 버스에 대한 정보또한 같이 표시하는 것입니다.  
정류장에 대한 버스의 번호와 남은 시간이 리스트형식으로 구현을 했다. (62-1 , 5) , (62-1,8) 이런식으로 총 하나의 버스당 두개의 정보가 나타나는것이 보인다. 빨리 오는 버스를 main에 표시하고 늦게 오는 버스를 sub에 표시를 해야한다.(해결! https://kimq1005.tistory.com/4)
